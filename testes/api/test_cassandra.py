...
class Cassandradb_teste(unittest.TestCase): 
    ...
    def test_put_modificando_nome_tipo(self):
        response = requests.put("http://localhost:5000/pokemons-cassandra/2", json={'id':2, 'name': 'pikachu', 'type': 'elétrico'})
        self.assertEqual(response.status_code, 200)
        documento_esperado = {"id": 2, "name": "pikachu", "type": "elétrico"}
        documento_criado = response.json()
        self.assertEqual(documento_esperado, documento_criado)
        response = requests.put("http://localhost:5000/pokemons-cassandra/2", json={'id': 2, 'name': 'charizard', 'type': 'fogo'})
